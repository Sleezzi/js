...
if (localStorage.getItem("volume") === null || localStorage.getItem("volume") === 'undefined') {
    ...
    volumeValue.innerHTML = (parseInt(localStorage.getItem("volume") * 100) < 10 ? "0" + parseInt(localStorage.getItem("volume") * 100) : parseInt(localStorage.getItem("volume") * 100)) + "%";
};

if (localStorage.getItem("loop") === null || localStorage.getItem("loop") === 'undefined') {
    localStorage.setItem("loop", false);
    audio.loop = false;
    loopImg.setAttribute("src", "https://sleezzi.github.io/img/loopOff.png");
} else {
    if (localStorage.getItem("loop") === "true") {
        audio.loop = true;
        loopImg.setAttribute("src", "https://sleezzi.github.io/img/loopOn.png");
    } else {
        audio.loop = false;
        loopImg.setAttribute("src", "https://sleezzi.github.io/img/loopOff.png");
    }
...
audioSelect.onchange = function() {
    const file = audioSelect.files[0];
    ...
    document.getElementById("title").innerHTML = "Player | " + file.name;
};
...
volumeRange.oninput = function() {
    ...
    volumeValue.innerHTML = (this.value < 10 ? "0" + this.value : this.value) + "%";
    volumeImg.setAttribute("src", (volumeRange.value === 0 ? "https://sleezzi.github.io/img/mute.png" : "https://sleezzi.github.io/img/audio.png"));
};

volumeImg.onmouseup = function() {
    if (audio.volume === 0) {
        audio.volume = localStorage.getItem("volume");
        volumeImg.setAttribute("src", "https://sleezzi.github.io/img/audio.png");
        volumeValue.innerHTML = parseInt(localStorage.getItem("volume") * 100) + "%";
        volumeRange.value = parseInt(localStorage.getItem("volume") * 100);
    } else {
        audio.volume = 0;
        volumeImg.setAttribute("src", "https://sleezzi.github.io/img/mute.png");
        volumeRange.style.marginLeft = "15px";
        volumeRange.value = 0;
